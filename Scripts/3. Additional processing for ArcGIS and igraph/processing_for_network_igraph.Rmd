...
network_master = network%>%
  mutate(
    ...
    system_size_agg = ifelse(pop_served <= 10000, "Small", "Large"))%>%
    
  select(c(system_name, pwsid, provider_type, source_name, source_type, purchased, average_source_usage:average_source_method, county:pop_served, system_size, system_size_agg, num_systems_served:num_sw_sources, gw_access:gw_compliant_w_dw_standards, swp_loop:cvp_loop))%>%
...
system_nodes = network_master%>%
  select(system_name, pwsid, provider_type, county, state_classification, pop_served, system_size, system_size_agg,  num_sw_sources, gw_access, gw_compliant_w_dw_standards)%>%
  unique()%>%
  rename(id = pwsid)
```
...
```{r}
save(network, all_nodes, file = "Data/Outputs/20260505_network_igraph.Rdata")
```
