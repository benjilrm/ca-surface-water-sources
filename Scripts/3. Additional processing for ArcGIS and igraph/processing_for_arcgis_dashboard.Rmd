...
```{r}
network = read.csv("Data/Outputs/20260505_network_joined.csv")
```
...
```{r}
sabl = st_read("C:/Users/benji/Desktop/COEQWAL/calsim-outcomes/Inputs/SABL/SABL/System_Area_Boundary_Layer.shp")%>%
...
network_w_lat_longs = network_w_lat_longs%>%
  left_join(sabl_points)%>%
  ...
  left_join(sabl_points, by = c("source_name" = "pwsid"))%>%
  mutate(source_lat = ifelse(is.na(sabl_lat), source_lat, sabl_lat),
         source_lon = ifelse(is.na(sabl_lon), source_lon, sabl_lon))%>%
  select(-c(sabl_lat, sabl_lon))
...
# Export
```{r}
st_write(connections_sf, "Data/Outputs/Shapefiles for dashboard/20260427_connections.shp", delete_dsn = T)

st_write(systems_sf, "Data/Outputs/Shapefiles for dashboard/20260427_systems.shp", delete_dsn = T)

st_write(jittered_sources, "Data/Outputs/Shapefiles for dashboard/20260427_sources.shp", delete_dsn = T)
```



```{r}
#st_read("Data/Outputs/Shapefiles for dashboard/20260423_connections.shp")%>%
connections_sf%>%
  data.frame()%>%
  filter(grepl("Alpine Springs County Water District Spring 01", source_name, ignore.case = T))%>%
  select(source_name, source_lon, source_lat)

#st_read("Data/Outputs/Shapefiles for dashboard/20260423_sources.shp")%>%
jittered_sources%>%
  filter(grepl("Alpine Springs County Water District Spring 01", source_name, ignore.case = T))%>%
  select(source_name)%>%
  mutate(source_lon = st_coordinates(.)[,1],
         source_lat = st_coordinates(.)[,2])%>%
  data.frame()
  #select(sorc_nm, sorc_lt, sorc_ln)
```
