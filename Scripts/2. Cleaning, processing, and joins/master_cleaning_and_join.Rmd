...
## Network data
```{r}
network_raw = read_excel("Data/Source/Network data/Final data for Benji May 5th.xlsx", na = "NA")
```
...
reviewed_swp = read.csv("Data/Source/Network data/SWP & CVP checks - 20251014_swp_systems_w_changes.csv")%>%
  mutate(swp = ifelse(Source == "Kern River" & PWSID == "CA1510040", 0, swp))
...
network_w_cleaned_swp_cvp = network_w_gw%>%
  rename(swp_og = swp_direct, cvp_og = cvp_direct, colorado_og = colorado_direct)%>%
...
# Export
```{r}
write.csv(network_w_looped_projects, "Data/Outputs/20260505_network_joined.csv", row.names = F)
```
